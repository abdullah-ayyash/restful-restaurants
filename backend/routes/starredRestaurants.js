...
router.post('/',(req,res) =>{
  ...
  const myRestaurant = ALL_RESTAURANTS.find((restaurant) => restaurant.id === id);
  const starredRestaurantReturn = {
    id: id,
    name: myRestaurant.name,
    comment: ''
  }
  res.json(starredRestaurantReturn);
...
router.delete('/:id',(req,res,next) =>{
  const { id } = req.params;
  const newArr =  STARRED_RESTAURANTS.filter(restaurant =>{
    return restaurant.id !== id;
  })
  if(STARRED_RESTAURANTS.length === newArr.length){
    res.status(404).send('Already removed');
  }
  STARRED_RESTAURANTS = newArr;
  res.sendStatus(200);
})
